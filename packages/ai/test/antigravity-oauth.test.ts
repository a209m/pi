...
describe.sequential("Antigravity OAuth", () => {
	...
	it("fails closed when refresh credentials are missing", async () => {
		const credential: OAuthCredential = {
			type: "oauth",
		...
		await expect(antigravityOAuth.refresh(credential, neverAbortedSignal)).rejects.toThrow(
			"Google Antigravity credentials do not include a refresh token; sign in again",
		);
	});
});
