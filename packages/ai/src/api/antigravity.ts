...
function consumeLine(text: string, final: boolean = false): { line: string; rest: string } | null {
	const lineBreakIndex = nextLineBreakIndex(text);
	if (lineBreakIndex === -1) {
		return null;
	}
	if (!final && text[lineBreakIndex] === "\r" && lineBreakIndex === text.length - 1) {
		return null;
	}
...
async function* iterateSseMessages(
	...
	try {
		...
		const finalEvent = decodeSseLine("", state);
		if (finalEvent) {
			yield finalEvent;
		}
	} finally {
	...
	(async () => {
		...
		try {
			if (!model.id.startsWith("gemini-")) {
				throw new Error(`Unsupported Antigravity model: ${model.id}`);
			}
			...
			const functionCallingMode = context.tools?.length
				? resolveGoogleFunctionCallingMode(context.tools, options?.toolChoice, false)
				: undefined;
			...
			const response = await retryProviderRequest(
				async () => {
					const nextResponse = await fetchFn(url, {
						method: "POST",
						headers: {
					...
					await options?.onResponse?.(
						{ status: nextResponse.status, headers: headersToRecord(nextResponse.headers) },
						model,
					);
					if (!nextResponse.ok) {
						const errorText = await nextResponse.text().catch(() => "");
						const error = new Error(
							`Antigravity error (${nextResponse.status}): ${errorText || nextResponse.statusText}`,
						) as Error & { status: number; headers: Headers };
						error.status = nextResponse.status;
						error.headers = nextResponse.headers;
						throw error;
					}
					return nextResponse;
				},
				...
				if (parts) {
					for (const part of parts) {
						if (isThinkingPart(part)) {
							...
							const toolCallId = needsNewId ? `${part.functionCall.name}_${uuidv7()}` : providedId;
